...
const Sidebar = ({ isConnected, activeTab, setActiveTab }) => {
    const navItems = [
        ...
        { icon: Video, label: 'Traffic Simulation', id: 'simulation' },
        { icon: MonitorDot, label: 'Real-time Monitor', id: 'monitor', subItems: ['Live Video Simulation'] },
        { icon: Route, label: 'Smart Routing', id: 'routing' },
        { icon: Camera, label: 'Camera Feed', id: 'camera' },
        { icon: Package, label: 'Delivery Management', id: 'delivery' },
        { icon: Truck, label: 'Fleet Monitoring', id: 'fleet' },
        { icon: MapPin, label: 'Delivery Assignment', id: 'assignment' },
    ...
    return (
        <aside className="w-64 bg-white dark:bg-[#0f172a] text-slate-500 dark:text-slate-400 border-r border-slate-200 dark:border-slate-800 flex flex-col h-screen fixed left-0 top-0 z-50 transition-[background-color,border-color] duration-300">
            <div className="p-6">
                <h1 className="text-slate-900 dark:text-white text-xl font-black flex items-center gap-2 tracking-tight">
                    Smart Traffic
                </h1>
                <div className={`flex items-center gap-2 mt-1 text-[10px] ${isConnected ? 'text-green-500' : 'text-red-500'} font-black uppercase tracking-widest`}>
                    <Wifi className={`h-3 w-3 ${isConnected ? 'animate-pulse' : ''}`} /> {isConnected ? 'Connected' : 'Offline'}
                </div>
            </div>
